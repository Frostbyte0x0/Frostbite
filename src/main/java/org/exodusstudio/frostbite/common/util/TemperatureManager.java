...
public class TemperatureManager {
    ...
    public static void updateEntityTemperature(LivingEntity entity) {
        ...
        if (entity.tickCount % 20 == 0) {
            ...
            if (isFrostbite(entity.level())) {
                float delta = 1.5f;
                if (DataHelper.getWeatherInfo(entity.level()).isWhiteouting) {
                    delta = 3.5f;
                } else if (DataHelper.getWeatherInfo(entity.level()).isBlizzarding) {
                    delta = 2.5f;
                }
    ...
    public static float calculateLiningDamping(Player player, float outerTempChange) {
    ...
    public static float calculateBlockTemperature(LivingEntity entity) {
        final float[] temp = {0};
    ...
    public static float updateInnerTemperature(float innerTemperature, float outerTemperature) {
