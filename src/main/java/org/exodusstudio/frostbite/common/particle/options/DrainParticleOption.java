...
public record DrainParticleOption(float roll) implements ParticleOptions {
    public static final MapCodec<DrainParticleOption> CODEC = RecordCodecBuilder.mapCodec((p_235920_) -> p_235920_.group(Codec.FLOAT.fieldOf("roll").forGetter((p_235922_) -> p_235922_.roll)).apply(p_235920_, DrainParticleOption::new));
    public static final StreamCodec<RegistryFriendlyByteBuf, DrainParticleOption> STREAM_CODEC;

    @Override
    public ParticleType<DrainParticleOption> getType() {
        return ParticleRegistry.DRAIN_PARTICLE.get();
    }

    static {
        STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.FLOAT, (p_319439_) -> p_319439_.roll, DrainParticleOption::new);
    }
}
